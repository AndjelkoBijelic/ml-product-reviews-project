...
"""
Šta smo novo uradili u ovom koraku? 

Model se fizički čuva u fajl.
Koristimo biblioteku joblib da model sačuvamo u fajl sentiment_model.pkl. Ovaj fajl:
sadrži ceo tok obrade i trenirani model;
može da se učita bilo kada i bilo gde bez ponovnog treniranja.

Koristimo ceo skup podataka za treniranje.
U ovoj fazi više ne delimo podatke na trening i test, već koristimo celokupni skup. 
Razlog je jednostavan: znamo da je ovaj model najbolji, 
pa želimo da on iskoristi sve dostupne podatke da nauči najviše što može.
"""